...
struct QuantizedConvNchwToConvNchw
  ...
  LogicalResult matchAndRewrite(linalg::Conv2DNchwFchwQOp op,
    ...
    if (!fZpIsZero) {
      ...
      auto expandTy = RankedTensorType::get({inputTy.getDimSize(0), 1,
...
public:
  void runOnOperation() override {
    ...
    patterns.add<QuantizedConvNhwcToConvNhwc, QuantizedConvNchwToConvNchw, QuantizedDepthwiseConvToDepthwiseConv>(
        context);
